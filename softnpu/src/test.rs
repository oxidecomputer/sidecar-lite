...
fn mcast_replication_basic() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v4_entry("224.1.1.1", &[1], &[2], 1);
...
fn mcast_source_filter_allows() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v6_entry("ff0e::1", &[1], &[2], 1);
...
fn mcast_replication_v4_outer() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v4_entry("238.1.1.1", &[1], &[2], 1);
    pipeline.add_ingress_mcast_mcast_replication_v4_entry(
        "set_port_bitmap",
        &key_buf,
        &param_buf,
...
fn mcast_egress_untagged_passthrough() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v6_entry("ff0e::1", &[1], &[], 1);
    pipeline.add_ingress_mcast_mcast_replication_v6_entry(
        "set_port_bitmap",
        &key_buf,
        &param_buf,
    ...
    let (key_buf, param_buf) = tbl_decap_ports_entry(1, &[1]);
    pipeline.add_egress_tbl_decap_ports_entry(
        "set_decap_ports",
        &key_buf,
        &param_buf,
        0,
...
fn mcast_egress_decap() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v6_entry(UNDERLAY_MCAST_DST, &[1, 2], &[2], 1);
    pipeline.add_ingress_mcast_mcast_replication_v6_entry(
        "set_port_bitmap",
        &key_buf,
        &param_buf,
    ...
    let (key_buf, param_buf) = tbl_decap_ports_entry(1, &[2]);
    pipeline.add_egress_tbl_decap_ports_entry(
        "set_decap_ports",
        &key_buf,
        &param_buf,
...
fn mcast_egress_multi_port_passthrough() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v6_entry("ff0e::1", &[1, 2], &[2], 1);
    pipeline.add_ingress_mcast_mcast_replication_v6_entry(
        "set_port_bitmap",
        &key_buf,
        &param_buf,
...
fn mcast_egress_decap_preserves_inner_udp() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v6_entry(UNDERLAY_MCAST_DST, &[2], &[2], 1);
    pipeline.add_ingress_mcast_mcast_replication_v6_entry(
        "set_port_bitmap",
        &key_buf,
        &param_buf,
...
fn mcast_egress_decap_vlan() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) = tbl_decap_ports_vlan_entry(1, &[2], 100);
    pipeline.add_egress_tbl_decap_ports_entry(
...
fn mcast_ingress_port_suppressed() -> Result<(), anyhow::Error> {
    ...
    let (key_buf, param_buf) =
        mcast_replication_v6_entry("ff0e::1", &[0, 1], &[], 1);
    pipeline.add_ingress_mcast_mcast_replication_v6_entry(
        "set_port_bitmap",
        &key_buf,
        &param_buf,
...
fn mcast_suppression_drops_when_only_group_zeroed() -> Result<(), anyhow::Error>
{
    ...
    let (key_buf, param_buf) =
        mcast_replication_v6_entry("ff0e::1", &[], &[2], 1);
    pipeline.add_ingress_mcast_mcast_replication_v6_entry(
        "set_port_bitmap",
        &key_buf,
        &param_buf,
...
//
// Lower endian encoded, meaning bit N (value 2^N) corresponds to port N, matching
// how p4rs arithmetic (shl_le, load_le, replicate) interprets bitvec
// storage.
//
// The bitmap is 128 bits (16 bytes).
...
//
// Key bytes are reversed to match the codegen's byte-reversed storage
// of header fields wider than 8 bits (hdr.ipv6.dst is 128 bits).
// Action has three parameters: external bitmap, underlay bitmap, and rid.
fn mcast_replication_v6_entry(
    ...
    param_buf.extend_from_slice(&rid.to_le_bytes());
...
// Decap port bitmap entry keyed on RID.
// The bitmap indicates which ports receive decapped traffic.
fn tbl_decap_ports_entry(rid: u16, ports: &[u16]) -> (Vec<u8>, Vec<u8>) {
    let key_buf = rid.to_le_bytes().to_vec();
    let param_buf = port_bitmap(ports);
    (key_buf, param_buf)
}

fn tbl_decap_ports_vlan_entry(
    rid: u16,
    ports: &[u16],
    vlan_id: u16,
) -> (Vec<u8>, Vec<u8>) {
    let key_buf = rid.to_le_bytes().to_vec();
    let mut param_buf = port_bitmap(ports);
    param_buf.extend_from_slice(&vlan_id.to_le_bytes());
    (key_buf, param_buf)
}
